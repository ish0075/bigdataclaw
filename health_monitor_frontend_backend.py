...
class FrontendBackendMonitor:
    ...
    SERVICES = {
        ...
        'nerve_frontend': {
            ...
            'port': 5174,
            'command': 'npm run dev',
            'directory': str(SCRIPT_DIR / 'nerve'),
        ...
        'nerve_backend': {
            ...
            'port': 3090,
            'command': 'python3 -m uvicorn main:app --reload --port 3090',
            'directory': str(SCRIPT_DIR / 'nerve' / 'server'),
        ...
        'api_server': {
            ...
            'port': 10000,
            'command': 'python3 enhanced_api.py',
            'directory': str(SCRIPT_DIR),
    ...
    def start_service(self, service_id):
        ...
        if self.check_port_in_use(service['port']):
            print(f"  ℹ️  Port {service['port']} already in use, assuming service is running.")
            if self.check_service_health(service_id):
                self.status[service_id] = 'running'
                return True
            else:
                print(f"  ⚠️  Port in use, but service is unhealthy. The monitor will attempt to restart it.")
                # The main loop will handle the restart logic. We just return False here.
                return False
        ...
        try:
            ...
            # Poll for health for up to 20 seconds
            healthy = False
            for i in range(20):
                # Check if process died
                if process.poll() is not None:
                    print(f"  ❌ {service['name']} process terminated unexpectedly.")
                    break
            ...
            if healthy:
                ...
                print(f"  ❌ {service['name']} failed to become healthy in time.")
                # Kill the process we started
                try:
                    os.killpg(os.getpgid(process.pid), signal.SIGTERM)
                    process.wait(timeout=5)
                except Exception as kill_e:
                    print(f"    Could not kill process {process.pid}: {kill_e}")
